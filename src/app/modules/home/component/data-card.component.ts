  ...
  template: `
    ...
    <div>
      ...
      <div *ngFor="let pkmn of pokemons; let idx = index">
        <div>
          <p (click)="showPokemonDetail(pkmn)" >{{pkmn.name}}, Clicca qui per maggiori informazioni </p>
          <img src="{{pkmn?.sprites?.back_default}}">
          <button class="btn btn-danger" *ngIf="deleteButton" (click)="deletePokemonOnTeam(idx, pkmn)"> Rimuovi </button>
        </div>

        
          <ng-template #fullTeam>
            <p> Hai troppi pokemon, liberane qualcuno </p>
            <img src="{{pkmn?.sprites?.back_default}}">
            <button class="btn btn-danger" (click)="deletePokemonOnTeam(idx, pkmn)"> Rimuovi </button>
          </ng-template>
